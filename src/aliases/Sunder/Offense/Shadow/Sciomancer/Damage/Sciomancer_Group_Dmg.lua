if snd.class == "Sciomancer" then
  ...
  function snd.attack_function()
    if not snd.waiting.queue then
      ...
      --			if not snd.checksomeAffs({"impeded","imbued"}, 2) and not snd.used.shield then gstring = "gravity imbue " ..snd.target..snd.sep.."gravity impede " ..snd.target..snd.sep else gstring = "" end
      if snd.repay < 2 and tonumber(gmcp.Char.Vitals.shadowprice) >= 3 and not snd.checkAff("gloom") then paystring =
        "cast repay" .. snd.sep elseif snd.repay < 1 and tonumber(gmcp.Char.Vitals.shadowprice) == 4 then paystring =
        "cast repay" .. snd.sep else paystring = "" end
      ...
      elseif not snd.checksomeAffs({ "clumsiness", "weariness", "lethargy", "paresis" }, 2) then
        if tonumber(gmcp.Char.Vitals.shadowprice) < 3 and not snd.checkAff("gloom") then
          astring = astring .. "shadowprice gloom " .. snd.target .. snd.sep .. "shadowprice ruin " ..
          snd.target .. snd.sep
